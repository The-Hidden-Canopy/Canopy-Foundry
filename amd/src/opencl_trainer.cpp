...
float cuda_attention_init_gain(const NativeRequest& request) {
    // Match trainer.cu's deterministic per-body Q/K gain range without
    // depending on CUDA's device-side RNG implementation.
    std::uint64_t hash = 1469598103934665603ULL;
    for (const char c : request.seat + ":" + request.version) {
        hash ^= static_cast<std::uint64_t>(static_cast<unsigned char>(c));
        hash *= 1099511628211ULL;
    }
    const double unit = static_cast<double>(hash >> 11) /
                        static_cast<double>(1ULL << 53);
    return static_cast<float>(0.85 + 0.35 * unit);
}
...
void initialize_weights(
    ...
    std::mt19937_64 generator(static_cast<std::uint64_t>(request.seed));
    ...
    const float hidden = static_cast<float>(model.hidden);
    const float intermediate = static_cast<float>(model.intermediate);
    const float qk_gain = cuda_attention_init_gain(request);
    const auto fan_in_limit = [](float fan_in, float gain) {
        return gain * std::sqrt(6.0f / fan_in);
    };
    init(model.embed, static_cast<std::size_t>(model.vocab) * model.hidden, 0.02f);
    ones(model.attn_norm, model.hidden);
    init(model.q_proj, model.hidden * model.hidden, fan_in_limit(hidden, qk_gain));
    init(model.k_proj, model.hidden * model.hidden, fan_in_limit(hidden, qk_gain));
    init(model.v_proj, model.hidden * model.hidden, fan_in_limit(hidden, 1.0f));
    init(model.o_proj, model.hidden * model.hidden, fan_in_limit(hidden, 1.0f));
    ones(model.ffn_norm, model.hidden);
    init(model.gate_proj, model.hidden * model.intermediate, fan_in_limit(hidden, 1.0f));
    init(model.up_proj, model.hidden * model.intermediate, fan_in_limit(hidden, 1.0f));
    init(model.down_proj, model.intermediate * model.hidden, fan_in_limit(intermediate, 1.0f));
    ones(model.final_norm, model.hidden);
    init(model.lm_head, model.vocab * model.hidden, 0.02f);
}
...
void set_loss(cl_kernel kernel, const Model& m, cl_mem labels, int valid_count) {
    int a = 0;
...
OpenCLTrainResult run_opencl_training(
    ...
    for (int t = 0; t < T; ++t) {
        ...
        if (labels[t] >= request.model.vocab_size)
            throw std::runtime_error("label id is outside model.vocab_size");
        if (labels[t] >= 0) ++valid_count;
    }
    if (valid_count == 0)
        throw std::runtime_error("OpenCL smoke dataset must contain at least one valid label");
    ...
    try {
        ...
        initialize_weights(runtime, model, request);
        ...
        set_loss(loss, model, label_buffer, valid_count);
        ...
        for (int step = 1; step <= request.training.max_steps; ++step) {
            ...
            average_loss /= static_cast<float>(valid_count * accumulation);
