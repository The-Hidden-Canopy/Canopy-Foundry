...
OpenCLRuntime create_opencl_runtime(int device_index) {
    ...
    check_opencl(clGetDeviceInfo(runtime.device, CL_DEVICE_MAX_WORK_GROUP_SIZE,
                                 sizeof(runtime.max_work_group_size), &runtime.max_work_group_size,
                                 nullptr),
    ...
    runtime.queue = clCreateCommandQueueWithProperties(
        runtime.context, runtime.device, queue_properties, &status);
#else
