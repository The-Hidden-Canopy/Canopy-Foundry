...
__kernel void loss_and_gradient(
    ...
    for (int token = 0; token < vocab; ++token) {
        ...
        dlogits[(size_t)t * vocab + token] = probability - (token == label ? 1.0f : 0.0f);
    }
}
...
__kernel void backward_full(
    ...
    for (int t = token_count - 1; t >= 0; --t) {
        ...
        for (int h = 0; h < hidden; ++h) {
            dh2[h] = (dy[h] * final_norm[h] -
                      h2v[h] * final_dot / ((float)hidden * final_rms * final_rms)) /
                     final_rms;
            dh1[h] = dh2[h];
            dnormf[h] = 0.0f;
        }
        ...
        float ffn_dot = 0.0f;
        for (int h = 0; h < hidden; ++h)
            ffn_dot += dnormf[h] * ffn_norm[h] * h1v[h];
        for (int h = 0; h < hidden; ++h) {
            const float dh = (dnormf[h] * ffn_norm[h] -
                              h1v[h] * ffn_dot / ((float)hidden * ffn_rms * ffn_rms)) /
                             ffn_rms;
            dh1[h] += dh;
        ...
        float attn_dot = 0.0f;
        for (int h = 0; h < hidden; ++h)
            attn_dot += dnorma[h] * attn_norm[h] * xv[h];
        const uint token = tokens[t];
        for (int h = 0; h < hidden; ++h) {
            const float dx = (dnorma[h] * attn_norm[h] -
                              xv[h] * attn_dot / ((float)hidden * attn_rms * attn_rms)) /
                             attn_rms;
